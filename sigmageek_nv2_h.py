...
def is_prime(n):
    ...
    if n % 2 == 0 or n <= 1:
        return False
    _root = int(n**0.5)+1
    _middle_root = int(_root/2)
    for i in range(3, _root , 2):   # only odd numbers
        if n % i == 0:
            return False
        if i >= _middle_root: print(f"Strong candidate: {n}")
    return True
...
def is_conditions_met_with(current_string, debug=True):
    if current_string:
        if is_palindrome(current_string): 
            ...
            with open("palindromes.txt") as p:
                ...
                if found or inverted_found:             
                    if found:
                        if self.slice_index != 0:
                            pi_digit = self.digits_counter - self.len_slice + ((self.slice_index) * (self.len_slice-self.n) + self.index_search - self.n) - 1
                        else:
                    ...
                    if inverted_found:
                        if self.slice_index != 0:
                            pi_digit =  self.digits_counter - self.len_slice + (self.slice_index * (self.len_slice - self.n) + (self.len_slice - 2*self.n - self.index_search)) -1
                        else:
...
# kwargs please...
def app(n=9, debug = 0, offset=0, slice_size=1024*1024, file_output="", file_input="", old_data = "", multi_file = False, file_index=file_index, file_name=file_name, digits_counter = 0):
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d", "--digits",
                        required=True,
                        default=None,
                        help="Number of digits to look for the prime palindrome")
    ...
    parser.add_argument("-mf", "--multifile",
                        required=False,
                        default=0,
                        help="O: False, 1: True. Multi File Support")
    file_index = 1
    file_name = f"//Volumes//SSD//bigpi//pi_dec_1t//pi_dec_1t_0{file_index}.txt"
    try:
