...
class newThreadSearch(threading.Thread):
    ...
    def run(self, debug=False):
        ...
        while hasdata:
            ...
            if debug: 
                print(f"[log] String: { current_string };  slice_index:{self.slice_index}; index_search:{self.index_search}; len(data): {self.len_slice}")
                print(f"[log] String: { inverted_search }; slice_index:{self.slice_index}; index_search:{(self.len_slice - self.index_search)}; len(data): {self.len_slice}")
            ...
            if is_conditions_met_with(current_string) or is_conditions_met_with(inverted_search):
                ...
                text_file.write(newThreadSearch.all_winners)
                text_file.close()
            self.index_search += 1
...
def app(n=9, debug = False, offset=0):
    ...
    slices_sizes = n*1000*10#*1024*1024
        
    print(f"Main: Finding prime palindromes inside pi with size n={n}")
        
    with open(file_name) as f:
        print("Main: Loading pi...")
        print(f"Main: Each '.' represents a slice with {slices_sizes} digits of pi being processed.")
        threads = []
        all_winners = ""
        old_data = ""
        while True:
            ...
            if newThreadSearch.hasChanged: 
                ...
                print(f"Main: {datetime.now()} All winners: { all_winners } " )
...
if __name__ == '__main__':
    ...
    app(21, debug=False, offset=384000)
    # unittest.main()
